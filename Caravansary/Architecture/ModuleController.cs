...
namespace Caravansary
{
    public class ModuleController : MarshalByRefObject, IModuleController
    {
        ...
        public Dictionary<string, ModuleInfo> CoreModules
        {
            get => _coreModules;
            set => _coreModules = value;
        }
        ...
        public string[] CoreModulesKeys
        {
            get
            {
                return CoreModules.Keys.ToArray();
            }
        }

        public ModuleInfo[] CoreModuleValues
        {
            get
            {
                return CoreModules.Values.ToArray();
            }
        }
        ...
        internal void RemoveModule(string name)
        {
            if (CoreModules.ContainsKey(name))
            {
                ...
                if (CoreModules.TryGetValue(name, out outVal))
                {
                    outVal.Loader.Stop();
                }
        ...
        public bool IsModuleActive(string name)
        {
            foreach (var item in CoreModulesKeys)
            {
                if (item == name && CoreModules[item].Loader.IsStarted)
                    return true;
            }
            return false;
        }
        ...
        public bool ScanDirectory(string path)
        {
            ...
            foreach (var dllDirectory in SpecificModuleDirectories)
            {
                ...
                try
                {
                    ...
                    CoreModules.Add(remoteLoader.Name, mi);
        ...
        public void StartCoreModule(string name)
        {
            if (CoreModules.ContainsKey(name))
            {
                var p = CoreModules[name].Loader;
                if (!p.IsStarted)
                {
                    p.Start();
                    OnModuleStarted?.Invoke(CoreModules[name]);
                }
            }
        }
        ...
        {
            if (CoreModules.ContainsKey(ModuleName))
            {
                var p = CoreModules[ModuleName].Loader;
                if (p.IsStarted)
                {
        ...
        public void StopCoreModule(string name)
        {
            if (CoreModules.ContainsKey(name))
            {
                ...
                {
                    p.Stop();
                    OnModuleStopped?.Invoke(CoreModules[name]);
                }
            }
        }
        ...
        internal void StopAllModules()
        {
            foreach (var item in CoreModulesKeys)
            {
                OnModuleStopped?.Invoke(CoreModules[item]);
                CoreModules[item].Loader.Stop();
            }
        }
